...
First, we scaled the features and used the elbow method to identify an appropriate number of clusters (k=4) and then ran k-means clustering on the dataset.  k-means clustering was selected since it was not known whether there were similarities across the wide range of universities in the UK, so running an unsupervised algorithm against the dataset would hopefully established whether any clusters appeared. (Running a test using DBScan required high epsilon values (0.5 with a normalised range of 0-1.0) and low minimum sample values (10 from 166) to begin to separate any cluster from the noise, which likely indicates that the dataset does not cluster well (either in general or using this density method).)
